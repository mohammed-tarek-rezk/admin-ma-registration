...
import {  useGetSingleModertatorQuery } from '../store/RTK/mainApi'
...
function ModeratorMembers() {
    let id = useParams().id
    let {data , isLoading} = useGetSingleModertatorQuery(id)
    ...
    useEffect(()=>{
        if(!isLoading){
            setMembers(filtering(data.data.members, community , status, accept))
        }
  ...
  <div className='flex flex-col md:flex-row justify-between items-center gap-3 my-6'>
      <div className='flex gap-4 items-center'>Members Count: <span className='text-xl bg-blue-500 text-white font-bold block px-3 rounded-md'>{data?.data?.members.length}</span> / <span className='text-xl bg-green-500 text-white font-bold block px-3 rounded-md'>{members?.length}</span></div>
...
export default ModeratorMembers